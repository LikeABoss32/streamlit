...
@st.cache_data
def load_workbook():
    file1 = "Clinic_db_with_metadata.xlsx"
    file2 = "Clinic_db.xlsx"
    try:
        return pd.read_excel(file1, sheet_name=None)
    except FileNotFoundError:
        try:
            return pd.read_excel(file2, sheet_name=None)
        except FileNotFoundError:
            st.error(f"Could not find either '{file1}' or '{file2}' in the app folder.")
            st.stop()

sheets = load_workbook()

# Get sheet dataframes
...
with tab_overview:
    ...
    if df_filtered.empty:
        ...
        with c1:
            ...
            if not daily.empty:
                if chart_type == "Line":
                    fig = px.line(daily, x="date", y="appointments", markers=True, title="Appointments per day")
                elif chart_type == "Bar":
                    fig = px.bar(daily, x="date", y="appointments", title="Appointments per day")
                else:
                    fig = px.area(daily, x="date", y="appointments", title="Appointments per day")
        ...
        with c2:
            ...
            if "app_reason" in df_filtered.columns:
                reasons_count = (
                    df_filtered["app_reason"].fillna("Unknown").value_counts().reset_index()
                )
                reasons_count.columns = ["reason", "count"]
                fig_r = px.bar(reasons_count.head(10), x="reason", y="count", title="Top 10 reasons")
    ...
    if "hour" in df_filtered.columns and df_filtered["hour"].notna().any():
        hour_counts = (
            df_filtered.groupby("hour")["appointmentid"].count().reset_index(name="appointments").sort_values("hour")
        )
        fig_h = px.bar(hour_counts, x="hour", y="appointments", title="Appointments by hour")
        fig_h.update_layout(xaxis_title="Hour of day",
        ...
        # Heatmap day x hour
        pivot = (
            df_filtered.pivot_table(index="day_name", columns="hour", values="appointmentid",
                                    aggfunc="count", fill_value=0)
            .reindex(["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"])
        )
        fig_heat = go.Figure(
            data=go.Heatmap(z=pivot.values, x=pivot.columns, y=pivot.index,
                            coloraxis="coloraxis")
        )
        fig_heat.update_layout(title="Heatmap – Day vs Hour",
                               xaxis_title="Hour",
...
with tab_time:
    ...
    if df_filtered.empty:
        ...
        season_counts = (
            df_filtered.groupby("season")["appointmentid"].count().reset_index(name="appointments")
        ).sort_values("appointments", ascending=False)
        if not season_counts.empty:
            col_t1, col_t2 = st.columns([1.5, 1])
            with col_t1:
                fig_s_bar = px.bar(season_counts, x="season", y="appointments", title="Season-wise appointments")
                fig_s_bar.update_layout(template="plotly_dark", xaxis_title="Season", yaxis_title="Appointments")
                st.plotly_chart(fig_s_bar, use_container_width=True)
            with col_t2:
                fig_s_pie = px.pie(season_counts, names="season", values="appointments", title="Season share")
        ...
        monthly = (
            ...
            .count().reset_index(name="appointments").sort_values(["year","month"])
        )
        if not monthly.empty:
            fig_m = px.line(monthly, x="month_name", y="appointments", color="season",
                            markers=True, line_group="year",
                            title="Monthly appointment trend by season")
            fig_m.update_layout(template="plotly_dark", xaxis_title="Month", yaxis_title="Appointments")
...
with tab_geo:
    ...
    if df_filtered.empty:
        ...
        geo = (
            df_filtered.groupby("city")["appointmentid"].count().reset_index(name="appointments")
        )
        ...
        if geo.empty:
            ...
            fig_geo = px.scatter_geo(geo, lat="lat", lon="lon",
                                     size="appointments", color="appointments",
                                     hover_name="city",
                                     projection="mercator",
                                     title="Patients / Appointments by City")
            fig_geo.update_geos(scope="asia", showland=True, landcolor="white",
            ...
            st.dataframe(geo.sort_values("appointments", ascending=False).reset_index(drop=True),
        ...
        with col_d1:
            if "departmentname" in df_filtered.columns:
                dept_stats = (
                    ...
                    .count().reset_index(name="appointments")
                    .sort_values("appointments", ascending=False)
                )
                fig_dept = px.bar(dept_stats, x="departmentname", y="appointments",
                                  title="Appointments by department")
                fig_dept.update_layout(template="plotly_dark", xaxis_tickangle=-30,
                                       xaxis_title="Department", yaxis_title="Appointments")
        ...
        with col_d2:
            if doctors_df is not None:
                ...
                doc_perf = doc_perf.merge(
                    doctors_df[["doctorid","doctorname","departmentid"]], on="doctorid", how="left"
                )
                if doctor_meta_df is not None:
                    doc_perf = doc_perf.merge(doctor_meta_df, on="doctorid", how="left")
                if departments_df is not None:
                    doc_perf = doc_perf.merge(departments_df[["departmentid","departmentname"]],
                                              on="departmentid", how="left", suffixes=("", "_dept"))
                if not doc_perf.empty:
                    fig_doc = px.scatter(doc_perf,
                                         x="appointments",
                    ...
                    fig_doc.update_layout(template="plotly_dark",
...
with tab_ml:
    ...
    if df_filtered.empty:
        ...
        daily = (
            df_filtered.groupby("date")["appointmentid"]
            .count().reset_index(name="appointments").sort_values("date")
        )
        if daily.shape[0] < 5:
            st.info("Not enough historical days for a meaningful forecast (need at least 5). Showing history only.")
            fig_hist = px.line(daily, x="date", y="appointments", markers=True,
            ...
            horizon = st.slider("Forecast horizon (days)", min_value=7, max_value=30, value=14)
            ...
            with col_f1:
                fig_fore_line = px.line(combined, x="date", y="appointments", color="type",
                                        markers=True, title="Daily appointments – history & forecast")
                fig_fore_line.update_layout(template="plotly_dark", xaxis_title="Date", yaxis_title="Appointments")
                st.plotly-chart(fig_fore_line, use_container_width=True)
            with col_f2:
                fig_fore_bar = px.bar(future_plot, x="date", y="appointments", title="Forecasted daily appointments (future only)")
                fig_fore_bar.update_layout(template="plotly_dark", xaxis_title="Date", yaxis_title="Forecasted appointments")
                st.plotly_chart(fig_fore_bar, use_container_width=True)

            next_7_total = future_plot.head(7)["appointments"].sum()
            st.markdown(f"**Projected appointments for next 7 days:** `{next_7_total:.0f}` (based on linear trend)")
        ...
        if not df_filtered.empty:
            if "season" in df_filtered.columns and "departmentname" in df_filtered.columns:
                ...
                if not cross.empty:
                    pivot_sd = cross.pivot(index="season", columns="departmentname", values="appointments").fillna(0)
                    fig_sd = go.Figure(data=go.Heatmap(z=pivot_sd.values, x=pivot_sd.columns, y=pivot_sd.index,
                                                      coloraxis="coloraxis"))
                    fig_sd.update_layout(title="Heatmap – Season vs Department (appointments)",
                                         xaxis_title="Department", yaxis_title="Season",
                                         coloraxis_colorscale="Blues", template="plotly_dark",
            ...
            with col_pf1:
                ...
                if not season_counts2.empty:
                    fig_season_bar2 = px.bar(season_counts2, x="season", y="appointments",
                                             title="Season-wise appointment volume")
                    fig_season_bar2.update_layout(template="plotly_dark", xaxis_title="Season", yaxis_title="Appointments")
                    st.plotly_chart(fig_season_bar2, use_container_width=True)
            with col_pf2:
                weekday_counts = (
                    df_filtered.groupby("day_name")["appointmentid"].count().reset_index(name="appointments")
                )
                if not weekday_counts.empty:
                    weekday_order = ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]
                    weekday_counts["day_name"] = pd.Categorical(weekday_counts["day_name"],
                                                                categories=weekday_order, ordered=True)
                    weekday_counts = weekday_counts.sort_values("day_name")
                    fig_weekday_pie = px.pie(weekday_counts, names="day_name", values="appointments",
        ...
        if rooms_df is not None and "noofbeds" in rooms_df.columns:
            total_beds2 = int(rooms_df["noofbeds"].fillna(0).sum())
            if not df_filtered.empty:
                recent_days = (
                    df_filtered.groupby("date")["appointmentid"].count().reset_index(name="appointments")
                    .sort_values("date", ascending=False)
                    .head(7)
                )
                avg_recent = recent_days["appointments"].mean()
                projected_bed_demand = min(total_beds2, avg_recent)
                fig_bed = go.Figure()
                fig_bed.add_trace(go.Bar(x=["Total Beds"], y=[total_beds2], name="Total Beds"))
                fig_bed.add_trace(go.Bar(x=["Projected Demand"], y=[projected_bed_demand], name="Projected Bed Demand (next 7 days avg)"))
                fig_bed.update_layout(barmode="group", title="Projected bed demand vs total capacity",
                                      template="plotly_dark", yaxis_title="Beds")
                st.plotly_chart(fig_bed, use_container_width=True)
                st.markdown(
                    f"- **Total beds:** `{total_beds2}`  ️\n"
                    f"- **Avg appointments (last 7 days in filter):** `{avg_recent:.1f}`  ️\n"
...
with tab_data:
    ...
    st.dataframe(df_filtered.reset_index(drop=True), use_container_width=True, height=500)
    st.markdown(f"Rows: **{df_filtered.shape[0]}**, Columns: **{df_filtered.shape[1]}**")
